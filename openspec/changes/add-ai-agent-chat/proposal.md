...
- 新增数据库表 `agent_conversations`（含 `userId` 列，按用户隔离）和 `agent_messages`，支持多会话管理、历史记录持久化、软删除
- 新增 `requireAuth()` 鉴权辅助，复用现有 JWT + `dp_auth` Cookie 认证体系；所有 `/api/agent/*` 路由强制登录，会话数据按当前用户 `userId` 隔离，未登录返回 401
...
- `agent-chat-api`: AI Agent 后端流式对话接口、LangChain Agent 编排、LLM 工厂、SSE 事件协议、鉴权（强制登录）
- `agent-tools`: 独立工具包模块，定义工具基类、注册中心、内置数据资产查询工具，支持按域扩展
- `agent-conversation-persistence`: 对话会话（按用户隔离）与消息的持久化（schema、migration、repository、service）
...
- **后端 API**: 新增 `src/app/api/agent/` 路由（SSE 流式 + 会话 CRUD），所有路由强制鉴权（复用现有 `auth.service` + `dp_auth` Cookie）
- **后端鉴权**: 新增 `requireAuth()` 辅助函数（复用现有 JWT 体系，无新认证机制）；与现有 `/api/auth/me` 鉴权写法一致
- **后端服务**: 新增 `src/app/server/agent/` 模块（agent.service / llm 工厂 / tools 工具包 / 工具注册中心），service 方法接收 `userId`
- **后端持久化**: 新增 `src/app/server/repositories/agent/`、`src/app/server/schemas/agent/`，新增 `src/app/db/schema/{mysql,sqlite}/agent-conversation.ts`（含 `userId` 列）、`agent-message.ts`，新增对应 relations 与 migration；repository 方法接收 `userId` 并按其过滤
- **依赖**: 需引入 `@langchain/core`、`@langchain/openai`、`@langchain/langgraph`（或 `langchain` 主包的 agent 能力）以及 `zod`（已有）。需确认与 Next.js 16 的兼容性
- **环境变量**: 新增 `LLM_PROVIDER`、`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`（或对应 provider 的密钥），更新 `.env.example`；鉴权沿用现有 `AUTH_JWT_SECRET`，无新增
